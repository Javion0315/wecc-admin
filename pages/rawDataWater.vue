				...
				<table class="min-w-full table-auto border-collapse text-sm" v-else>
					...
					<thead>
						<tr class="bg-gray-400 text-white text-left">
							...
							<th class="w-36 min-w-[100px] py-2 px-3">Value</th>
					...
					<tbody>
						...
						>
							...
							<th class="w-36 min-w-[100px] py-2 px-3">{{ item.value }}</th>
...
<script>
import { getAdminRawDataWater } from "@/api/rawData";
...
export default {
	...
	methods: {
		getData() {
			...
			getAdminRawDataWater(payLoad)
				.then((res) => {
					this.rawList = res.data.dataWater;
